...
@NamedQueries({
		...
		@NamedQuery(name = "CertificationModel.checkbyIdTocProperty", query = "Select cm from CertificationModel cm Join cm.tocBean t Join cm.propertyBean p where  cm.id=:cm_identifier and t.id =:toc_identifier and p.class_=:p_identifier"),
...
public class CertificationModel implements Serializable {
	...
	@OneToMany(mappedBy = "certificationModel", fetch = FetchType.LAZY)
	...
	public String toString() {
		return "\n\n\n\n\n\n\n\n\n" + this.id + " TOC:" + this.tocBean.getId()
				+ "-" + tocBean.getTocid() + "\nProperty:"
				+ propertyBean.getClass_() + "/n ACollector n"
				+ this.abstractCollectors.size() + "\n Collector n";// +this.collectors.size();
	}
